...
def test_config_path_falls_back_to_home(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    monkeypatch.delenv("XDG_CONFIG_HOME", raising=False)
    monkeypatch.setenv("HOME", str(tmp_path))
    assert profile.config_path() == tmp_path / ".config" / "visualgen-mcp" / "config.toml"


def test_config_path_ignores_empty_xdg(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
